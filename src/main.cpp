...
void captureImages(MyImage *m){
	cout << "Starting image capture mode"<<endl;
	int captureNumber = 0;
	...
	for(;;){
		m->cap >> m->src;
		namedWindow("Recording",CV_WINDOW_KEEPRATIO);
		flip(m->src,m->src,1);
		pyrDown(m->src,m->srcLR);
		blur(m->srcLR,m->srcLR,Size(3,3));
		cvtColor(m->srcLR,m->srcLR,ORIGCOL2COL);
		produceBinaries(m);
		imshow("Recording", m->bw);
		char keyStroke = cv::waitKey(30);
		if(keyStroke == char('c')){
			std::string s;
			std::stringstream out;
			out << captureNumber;
			imwrite(filePath+"/"+alphabet+"/"+out.str()+".jpg",m->bw,compression_params);
			captureNumber++;
		}
		else if(keyStroke == char('q')){
...
int main(){
	...
	int frameNumber = 0;
	basicHSR hsr;
	cout << "Enter the hand sign dataset path : ";
	cin >> hsr.sampleDatasetPath;
	cout << "Enter number of classes : ";
	cin >> hsr.classes;
	cout << "Enter number of samples to train on : ";
	cin >> hsr.trainSamples;
	cout << "Enter number of samples to test on : ";
	cin >> hsr.testSampleNumber;
	cout << "Enter starting number of test sample : ";
	cin >> hsr.testSampleStart;
	hsr.init();
	init(&m);
	m.cap >> m.src;
	...
	for(;;){
		...
		blur(m.srcLR,m.srcLR,Size(3,3));
		cvtColor(m.srcLR,m.srcLR,ORIGCOL2COL);
		namedWindow("Image_After_Processing", CV_WINDOW_KEEPRATIO);
		imshow("Image_After_Processing", m.srcLR);
		produceBinaries(&m);
		...
		char keyStroke = cv::waitKey(30);
		if(keyStroke == char('q')) break;
		if(keyStroke == char('r')){
			captureImages(&m);
		}
		hsr.classify(new IplImage(m.bw), 1, true);
	}
	destroyAllWindows();
	out.release();
	m.cap.release();
